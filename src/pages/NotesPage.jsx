...
// --- Komponen Item (Tidak ada perubahan) ---
...
function NotesPage({ session }) { 
    ...
    // === PERUBAHAN: Mendengarkan 'updateTrigger' dari store ===
    const openFolderEditModal = useFolderEditModalStore((state) => state.openModal);
    const updateTrigger = useFolderEditModalStore((state) => state.updateTrigger);
    ...
    useEffect(() => {
        async function fetchTopLevelFolders() {
            ...
            setLoading(true);
            try {
                // Panggil RPC Supabase untuk efisiensi
    ...
    // === PERUBAHAN: Menambahkan 'updateTrigger' sebagai dependency ===
    // Ini akan menjalankan ulang useEffect setiap kali folder di-update
