...
function WebCollectionDetailPage({ session }) {
  const { id } = useParams(); // ID koleksi induk
  const navigate = useNavigate();
  const [collection, setCollection] = useState(null); // Data koleksi induk
  const [items, setItems] = useState([]); // Daftar sub-link (web_items)
  const [categories, setCategories] = useState([]); // Daftar kategori untuk edit koleksi
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [isEditingCollection, setIsEditingCollection] = useState(false); // Mode edit untuk koleksi induk

  // State untuk form edit koleksi induk
  ...
  // --- Fungsi Pengambilan Data Koleksi & Itemnya, serta Kategori ---
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!id) {
          setError("ID koleksi tidak ditemukan di URL.");
          setLoading(false);
          return;
        }
        ...
        if (collectionError) {
          if (collectionError.code === 'PGRST116') {
            setError("Koleksi tidak ditemukan atau Anda tidak memiliki akses.");
          } else {
        ...
        // Ambil web_items yang terkait dengan koleksi ini
  ...
  const handleUpdateCollection = async (e) => {
    ...
    if (!user) {
      alert("Anda harus login untuk mengedit koleksi.");
      return;
    }
    ...
    try {
      const { data, error: updateError } = await supabase
        ...
        .update({
          ...
          category_id: editCollectionCategory,
        })
        .eq('id', id)
        .eq('user_id', user.id)
        .select(`
  ...
  // --- Fungsi Tambah Item Web Baru (Sub-Link) ---
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          // order bisa diisi nanti jika kita butuh pengurutan spesifik
        })
        .select();
  ...
  // --- Fungsi Hapus Item Web (Sub-Link) ---
  const handleDeleteItem = async (itemId) => {
    if (!window.confirm("Apakah Anda yakin ingin menghapus item ini dari koleksi?")) return;
    if (!user) {
    ...
    try {
      // Pastikan user memiliki akses ke koleksi induk
      const { error } = await supabase
        .from('web_items')
        .delete()
        .eq('id', itemId)
        .eq('collection_id', id); // Memastikan item adalah bagian dari koleksi ini
  ...
  if (loading) {
    return <div className="p-6 text-center">Memuat detail koleksi...</div>;
  }
  ...
  if (!collection) {
    return <div className="p-6 text-center text-gray-600 dark:text-gray-400">Koleksi tidak ditemukan.</div>;
  }
  ...
  return (
    <div className="p-6 bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-900 dark:text-gray-100">
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-6 max-w-3xl mx-auto">
        ...
        {/* Header Koleksi Induk: Tampilan atau Edit */}
        {!isEditingCollection ? (
          <>
            ...
            {collection.description && (
              <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap mb-6">{collection.description}</p>
            )}
          </>
        ) : (
          <form onSubmit={handleUpdateCollection} className="space-y-4 mb-8">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setEditCollectionCategory(e.target.value)}
                className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              >
        ...
        {/* Form Tambah Item Baru ke Koleksi Ini */}
          ...
          <form onSubmit={handleAddItem} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="Judul link di dalam koleksi"
                className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              <label htmlFor="newItemUrl" className="block text-sm font-medium mb-1">URL Item</label>
              <input
                type="url"
                id="newItemUrl"
                value={newItemUrl}
                onChange={(e) => setNewItemUrl(e.target.value)}
                placeholder="https://contoh.com/artikel-spesifik"
                className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
                required
              />
            </div>
            <div>
              <label htmlFor="newItemDescription" className="block text-sm font-medium mb-1">Deskripsi Item (Opsional)</label>
              <textarea
                id="newItemDescription"
                value={newItemDescription}
                onChange={(e) => setNewItemDescription(e.target.value)}
                placeholder="Penjelasan singkat tentang item link ini..."
                rows="2"
                className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded-md bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
              ></textarea>
            </div>
            ...
            >
              Tambah Item ke Koleksi Ini
            </button>
          </form>
        </div>

        {/* Daftar Item dalam Koleksi Ini */}
        <div className="bg-gray-50 dark:bg-gray-700 shadow rounded-lg p-5">
          ...
          {items.length === 0 ? (
            <p className="text-center text-gray-600 dark:text-gray-400">Belum ada item dalam koleksi ini. Tambahkan yang pertama di atas!</p>
          ) : (
            <div className="space-y-4">
              {items.map((item) => (
                <div key={item.id} className="border border-gray-200 dark:border-gray-600 rounded-lg p-4 bg-white dark:bg-gray-800 flex flex-col">
                  ...
                  {item.description && (
                    <p className="text-gray-700 dark:text-gray-300 text-sm mb-2 whitespace-pre-wrap">{item.description}</p>
                  )}
