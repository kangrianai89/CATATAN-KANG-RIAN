...
function AssetDetailPage() {
  ...
  const handleUpdateAsset = async (e) => {
    e.preventDefault();
    setIsSaving(true);
    let newImagePath = asset.image_path;
    ...
    try {
      ...
      if (newImageFile) {
        ...
        if (asset.image_path) {
          ...
          if (removeError) console.error("Gagal hapus gambar lama:", removeError); // Tampilkan di console saja
        }
      ...
      const { data, error } = await supabase
        ...
        .update({
          ...
          code: editCode,
          image_path: newImagePath, // <-- Simpan path gambar yang baru
        })
        .eq('id', id)
        .select()
        .single();
      ...
      setAsset(data);
      if (newImageFile && data.image_path) { // Jika ada gambar baru, perbarui URLnya
        const { data: urlData } = supabase.storage.from('asset-images').getPublicUrl(data.image_path);
        setImageUrl(urlData.publicUrl);
      }
  ...
  if (!asset) return <div className="dark:text-white p-8">Aset tidak ditemukan.</div>;
  ...
  const renderContent = () => (
    <div className="space-y-8">
      ...
      <div>
        ...
        {imageUrl && !isEditing && (
            <img src={imageUrl} alt={asset.title} className="max-w-full md:max-w-lg rounded-lg shadow-md border" />
        )}
        {isEditing && (
            <>
                {imageUrl && <img src={imageUrl} alt="Gambar saat ini" className="w-48 h-auto rounded-lg shadow-md border mb-4" />}
         ...
         {!imageUrl && !isEditing && <p className="text-gray-500 italic">Tidak ada gambar.</p>}
      </div>
    ...
    <div>
      <form onSubmit={handleUpdateAsset}>
        <div className="flex justify-between items-center mb-6">
          {isEditing ? (
            <input type="text" value={editTitle} onChange={(e) => setEditTitle(e.target.value)} className="text-3xl font-bold dark:text-white bg-transparent border-b-2 dark:border-gray-600 focus:outline-none focus:border-blue-500" />
          ) : (
          ...
          <div className="flex items-center gap-2">
            ...
            {isEditing ? (
              <>
                <button type="button" onClick={() => setIsEditing(false)} className="px-4 py-2 text-sm rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700">Batal</button>
